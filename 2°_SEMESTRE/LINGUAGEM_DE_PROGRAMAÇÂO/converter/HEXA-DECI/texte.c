...
char** split_string(const char* str, const char* delim)
{
    ...
    size_t vec_size = 0; // Size of the vector
    char** vec = NULL; // Vector of strings

    // Split the string and store the tokens in the vector
    for (char* token = strtok_r(copy, delim, &saveptr); token != NULL;
         token = strtok_r(NULL, delim, &saveptr)) {
        vec_size++; // Increase the size of the vector
        vec = realloc(vec, vec_size * sizeof(char*)); // Allocate memory for the new token
        vec[vec_size - 1] = strdup(token); // Store the token in the vector
    }

    free(copy); // Free the memory allocated for the copy of the input string
...
int main(void)
{
    ...
    // Free the memory allocated for the vector and its elements
    for (size_t i = 0; i < 5; i++) {
        free(vec[i]);
    }
    free(vec);
